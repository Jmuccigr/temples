...
try:
	...
	sys.exit(dateTimeObj.strftime("%Y-%m-%d %H:%M:%S") + " temples file is not valid JSON.")

if os.stat(basedir + 'pelagios.json').st_size < 100000:
...
#                 outputText += 'ov:compassDirection <http://vocab.getty.edu/page/aat/' + settingText + '> ;\n'
            ...
            if record['properties'].get('digitalaugustanrome') != '' and record['properties'].get('digitalaugustanrome') is not None:
                outputText += 'skos:exactMatch <https://www.digitalaugustanrome.org/records/' + record['properties']['digitalaugustanrome'] + '/> ;\n'
