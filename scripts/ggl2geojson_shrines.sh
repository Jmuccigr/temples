...
me=$(whoami)
key="1MTaQnTbRu1hxuw-Ggfch_tvgHwc12jvEo4jr1zerZL8"
dest="/Users/$me/Documents/github/local/shrines"
...
then
	...
	jq '{type: .type, features: .features}' "$temp/shrines.json" > "$dest/shrines.json"

	# For mapping, save a second copy with only entries including coordinates
	head -n 3 "$dest/shrines.json" > "$temp/shrines.json"
	text=$(jq -c '.features[] | select (.geometry.coordinates != "")' "$dest/shrines.json" | tr '\n' ',')
	text="$text ]}"
	echo $(echo $text | sed 's/, \]\}/\]\}/') >> "$temp/shrines.json"
	jq '.' "$temp/shrines.json" > "$dest/maps/shrines.json"
else
